    ...
    <div class="modal-content" id="formulario">
      ...
      <div class="modal-body">
        ...
        <?php
                ...
                <select class="custom-select" id="cliente" name="cliente" required>
                    ...
                    <?php
                        if($total > 0) {
                            do {
                    ?>
                        <option value="<?=$linha['idclient']?>"><?=$linha['name']?></option>
                    <?php
            ...
            <?php
              $con = DBConnect();
              $select = DBQuery('veiculo'," inner join modelo on modelo_idmodelo = idmodelo inner join categoria on categoria_idcategoria = idcategoria;",
                                'idveiculo,name,nome');
              $linha = mysqli_fetch_assoc($select);
              // calcula quantos dados retornaram
              $total = mysqli_num_rows($select);
                ...
                <select class="custom-select" id="veiculo" name="veiculo" required>
                    ...
                    <?php
                        if($total > 0) {
                            do {
                    ?>
                        <option value="<?=$linha['idveiculo']?>"><?=$linha['name']?> <?=$linha['nome']?></option>
                    <?php
                        }while($linha = mysqli_fetch_assoc($select));
                            // fim do if 
                        }
